"""Readable, self-contained ANYmal C velocity environment."""
...
from .robot_cfg import ANYMAL_C_ACTION_SCALE, get_anymal_c_robot_cfg


_BASE_BODY = "base"
_FOOT_SITES = ("LF_foot", "RF_foot", "LH_foot", "RH_foot")
_FOOT_GEOMS = ("LF_FOOT", "RF_FOOT", "LH_FOOT", "RH_FOOT")


def make_env_cfg(*, rough: bool, play: bool = False) -> ManagerBasedRlEnvCfg:
    """Create the flat or rough ANYmal C velocity environment."""
    ...
    actions = {
        "joint_pos": JointPositionActionCfg(
            entity_name="robot",
            actuator_names=(".*",),
            scale=ANYMAL_C_ACTION_SCALE,
            use_default_offset=True,
        )
    }

    command_ranges = UniformVelocityCommandCfg.Ranges(
        lin_vel_x=(-1.0, 2.0) if play and not rough else (-1.0, 1.0),
        lin_vel_y=(-1.0, 1.0),
        ang_vel_z=(-1.0, 1.0) if play and not rough else (-0.5, 0.5),
    ...
    commands: dict[str, CommandTermCfg] = {
        "twist": UniformVelocityCommandCfg(
            ...
            viz=UniformVelocityCommandCfg.VizCfg(z_offset=0.7),
        )
    }
    ...
    return ManagerBasedRlEnvCfg(
        scene=SceneCfg(
            ...
            num_envs=1,
            extent=2.0,
            entities={"robot": get_anymal_c_robot_cfg()},
