...
class VelocityDuckMini(mjx_env.MjxEnv):
    """ Environment to train the Duck Mini to follow velocity commands"""
    def __init__(
        self,
        ...
        self._xml_path  = "main/environments/velocity_duck_mini/model.xml"
        ...
        self._command_range_linvel_x   = [-0.5, 1.0]
        ...
        self._push_magnitude_range = [0.1, 0.5]
        ...
        for joint_id in range(self._mj_model.njnt):
            ...
            # Handle free joint
            if "trunk_freejoint" in joint_name:
    ...
    def _reward_pose(self, data, info) -> jax.Array:
        ...
        std_standing = 0.05
        std_moving = 0.35
        std_neck = 0.05  # Tight constraint — keep head still during locomotion.

        n_joints = len(self._start_joints)
        per_joint_std = jp.array([std_moving] * (n_joints - 4) + [std_neck] * 4)
        ...
        std = jp.where(speed < velocity_active_threshold, std_standing, per_joint_std)
...
locomotion.register_environment('velocity_duck_mini', VelocityDuckMini, default_config)
