...
class VelocityDuckMini_Play:
    def __init__(self):
        options_path = "main/environments/velocity_duck_mini/options.json"
        xml_path     = "main/environments/velocity_duck_mini/model.xml"
        ...
        # Create agent brain
        brain_neurons = network_model.load_params("models/velocity_duck_mini/model")
        ...
        self._reset_noise_scale    = training_env._joint_reset_noise
        ...
        self._command_step_linvel_x    = 0.05
    ...
    def reset(self, data: mujoco.MjData, rng: jax.Array) -> Tuple[mujoco.MjData, jax.Array]:
        ...
        joint_noise = self._reset_noise_scale
...
def _make_training_env(mjx_options: dict[str, Any]) -> VelocityDuckMini:
    env_cfg = mp_registry.get_default_config("velocity_duck_mini")
    env_cfg["impl"] = mjx_options["impl"]
    env_cfg["ctrl_dt"] = mjx_options["ctrl_dt"]
    env_cfg["sim_dt"] = mjx_options["sim_dt"]
    return VelocityDuckMini(config=env_cfg)
