...
class VelocityHumanoid_Play:
    def __init__(self):
        options_path = "main/environments/velocity_humanoid/options.json"
        xml_path     = "main/environments/velocity_humanoid/model.xml"
        ...
        # Create agent brain
        brain_neurons = network_model.load_params("models/velocity_humanoid/model")
...
def _make_training_env(mjx_options: dict[str, Any]) -> VelocityHumanoid:
    env_cfg = mp_registry.get_default_config("velocity_humanoid")
    env_cfg["impl"] = mjx_options["impl"]
    env_cfg["ctrl_dt"] = mjx_options["ctrl_dt"]
    env_cfg["sim_dt"] = mjx_options["sim_dt"]
    return VelocityHumanoid(config=env_cfg)
