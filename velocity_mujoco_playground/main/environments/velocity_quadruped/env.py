...
class VelocityQuadruped(mjx_env.MjxEnv):
    """ Environment to train a Quadruped to follow velocity commands"""
    def __init__(
        self,
        ...
        self._xml_path  = "main/environments/velocity_quadruped/model.xml"
        self._mj_model  = mujoco.MjModel.from_xml_path(self._xml_path)
        self._mj_model.opt.timestep = self.sim_dt
        self._mjx_model = mjx.put_model(self._mj_model, impl=self._config.impl)

        # Action scale
        self._action_scale = 1.00
        ...
        for joint_id in range(self._mj_model.njnt):
            ...
            # Handle free joint
            if "quadruped_root" in joint_name:
        ...
        self._feet_site_id = jp.array([
            self._mj_model.site("quadruped_back_right_foot").id,
            self._mj_model.site("quadruped_back_left_foot").id,
            self._mj_model.site("quadruped_front_right_foot").id,
            self._mj_model.site("quadruped_front_left_foot").id,
        ])
    ...
    def _calculate_obs(self, data: mjx.Data, info: dict[str, Any]) -> mjx_env.Observation:
        ...
        root_height = data.qpos[self._agent_qpos_ids[2]]
    ...
    def _calculate_done(self, data: mjx.Data) -> jax.Array:
        # Terminate if the angle of the quadruped is too large relative to the z-axis
        torso_angle = jp.ravel(data.site_xmat[self._agent_site_id])[8]
        healthy     = torso_angle > self._agent_healthy_angle
    ...
    def _get_feet_contacts(self, data):
        # Foot order: FR, FL, RR, RL.
        return jp.array([
            mjx_env.get_sensor_data(self._mj_model, data, "front_right_foot_floor_contact")[0] > 0,
            mjx_env.get_sensor_data(self._mj_model, data, "front_left_foot_floor_contact")[0] > 0,
            mjx_env.get_sensor_data(self._mj_model, data, "back_right_foot_floor_contact")[0] > 0,
            mjx_env.get_sensor_data(self._mj_model, data, "back_left_foot_floor_contact")[0] > 0,
        ])

    def _get_feet_vel(self, data: mjx.Data) -> jax.Array:
        front_right_foot_linvel= mjx_env.get_sensor_data(self._mj_model, data, "front_right_foot_linvel")
        front_left_foot_linvel= mjx_env.get_sensor_data(self._mj_model, data, "front_left_foot_linvel")
        back_right_foot_linvel= mjx_env.get_sensor_data(self._mj_model, data, "back_right_foot_linvel")
        back_left_foot_linvel= mjx_env.get_sensor_data(self._mj_model, data, "back_left_foot_linvel")
        return jp.stack([front_right_foot_linvel, front_left_foot_linvel, back_right_foot_linvel, back_left_foot_linvel], axis=0)
...
locomotion.register_environment('velocity_quadruped', VelocityQuadruped, default_config)
