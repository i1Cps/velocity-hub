...
class VelocityQuadruped_Play:
    def __init__(self):
        options_path = "main/environments/velocity_quadruped/options.json"
        xml_path     = "main/environments/velocity_quadruped/model.xml"
        ...
        # Create agent brain
        brain_neurons = network_model.load_params("models/velocity_quadruped/model")
        ...
        self._joint_reset_noise    = training_env._joint_reset_noise
        ...
        self._command_step_linvel_x    = 0.1
    ...
    def preprocess_actions(self, actions) -> jp.ndarray:
        return actions * self._action_scale
    ...
    def calculate_obs(self, data: mjx.Data) -> jp.ndarray:
        ...
        joint_pos   = data.qpos[self._agent_qpos_ids][7:]
...
def _make_training_env(mjx_options: dict[str, Any]) -> VelocityQuadruped:
    env_cfg = mp_registry.get_default_config("velocity_quadruped")
    env_cfg["impl"] = mjx_options["impl"]
    env_cfg["ctrl_dt"] = mjx_options["ctrl_dt"]
    env_cfg["sim_dt"] = mjx_options["sim_dt"]
    return VelocityQuadruped(config=env_cfg)
