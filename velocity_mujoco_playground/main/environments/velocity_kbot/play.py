...
class VelocityKbot_Play:
    def __init__(self):
        options_path = "main/environments/velocity_kbot/options.json"
        xml_path     = "main/environments/velocity_kbot/model.xml"
        ...
        # Create agent brain
        brain_neurons = network_model.load_params("models/velocity_kbot/model")
        ...
        self._agent_healthy_height = training_env._agent_healthy_height
        ...
        self._command_step_linvel_x    = 0.20
        self._command_step_linvel_y    = 0.10
        self._command_step_angvel_yaw  = 0.2
    ...
    def calculate_termination(self, data: mjx.Data):
        ...
        healthy = jp.logical_and(
            torso_height > self._agent_healthy_height,
            torso_orientation > self._agent_healthy_angle,
        )
...
def _make_training_env(mjx_options: dict[str, Any]) -> VelocityKbot:
    env_cfg = mp_registry.get_default_config("velocity_kbot")
    env_cfg["impl"] = mjx_options["impl"]
    env_cfg["ctrl_dt"] = mjx_options["ctrl_dt"]
    env_cfg["sim_dt"] = mjx_options["sim_dt"]
    return VelocityKbot(config=env_cfg)
