...
class VelocityAnymalC_Play:
    def __init__(self):
        options_path = "main/environments/velocity_anymal_c/options.json"
        xml_path     = "main/environments/velocity_anymal_c/model.xml"
        ...
        # Create agent brain
        brain_neurons = network_model.load_params("models/velocity_anymal_c/model")
...
def _make_training_env(mjx_options: dict[str, Any]) -> VelocityAnymalC:
    env_cfg = mp_registry.get_default_config("velocity_anymal_c")
    env_cfg["impl"] = mjx_options["impl"]
    env_cfg["ctrl_dt"] = mjx_options["ctrl_dt"]
    env_cfg["sim_dt"] = mjx_options["sim_dt"]
    return VelocityAnymalC(config=env_cfg)
