...
class VelocityAnymalC(mjx_env.MjxEnv):
    """ Environment to train the Anymal C to follow velocity commands"""
    def __init__(
        self,
        ...
        self._xml_path  = "main/environments/velocity_anymal_c/model.xml"
        self._mj_model  = mujoco.MjModel.from_xml_path(self._xml_path)
        self._mj_model.opt.timestep = self.sim_dt
        self._mjx_model = mjx.put_model(self._mj_model, impl=self._config.impl)

        # Action scale
        self._action_scale = 0.50
        ...
        self._command_range_linvel_x   = [-1.5, 2.0]
        self._command_range_linvel_y   = [-1.0, 1.0]
        ...
        self._action_rate_cost_weight       = -0.01
        ...
        self._feet_site_id = jp.array([
            self._mj_model.site("LF_foot").id,
            self._mj_model.site("RF_foot").id,
            self._mj_model.site("LH_foot").id,
            self._mj_model.site("RH_foot").id,
        ])
    ...
    def _get_feet_vel(self, data: mjx.Data) -> jax.Array:
        front_left_foot_vel = mjx_env.get_sensor_data(self._mj_model, data, "LF_foot_global_linvel")
        front_right_foot_vel = mjx_env.get_sensor_data(self._mj_model, data, "RF_foot_global_linvel")
        back_left_foot_vel = mjx_env.get_sensor_data(self._mj_model, data, "LH_foot_global_linvel")
        back_right_foot_vel = mjx_env.get_sensor_data(self._mj_model, data, "RH_foot_global_linvel")
    ...
    def _get_feet_contacts(self, data: mjx.Data) -> jax.Array:
        return jp.array([
            mjx_env.get_sensor_data(self._mj_model, data, "LF_foot_floor_contact")[0] > 0,
            mjx_env.get_sensor_data(self._mj_model, data, "RF_foot_floor_contact")[0] > 0,
            mjx_env.get_sensor_data(self._mj_model, data, "LH_foot_floor_contact")[0] > 0,
            mjx_env.get_sensor_data(self._mj_model, data, "RH_foot_floor_contact")[0] > 0,
        ])
...
locomotion.register_environment('velocity_anymal_c', VelocityAnymalC, default_config)
