...
GAIT_PHASES = {
    ...
    0: jp.array([0.0, jp.pi, jp.pi, 0.0]),
    # walk (staggered diagonals)
    1: jp.array([0.0, 0.5 * jp.pi, jp.pi, 1.5 * jp.pi]),
}

class VelocitySpot(mjx_env.MjxEnv):
    """ Environment to train Spot to follow velocity commands"""
    def __init__(
        self,
        ...
        self._xml_path  = "main/environments/velocity_spot/model.xml"
        self._mj_model  = mujoco.MjModel.from_xml_path(self._xml_path)
        self._mj_model.opt.timestep = self.sim_dt
        self._mjx_model = mjx.put_model(self._mj_model, impl=self._config.impl)

        # Action scale
        self._action_scale = 0.10

        # Noise applied to robot's start position
        self._joint_reset_noise = 0.0

        # Observation noise scale
        self._joint_pos_noise_scale = 0.01
        self._joint_vel_noise_scale = 1.50
        self._gravity_noise_scale   = 0.05
        self._linvel_noise_scale    = 0.5
        self._gyro_noise_scale      = 0.2

        # Velocity command ranges
        self._command_range_linvel_x   = [-1.0, 2.0]
        ...
        self._agent_healthy_height = 0.00

        # Reward weights
        self._tracking_linvel_reward_weight = 2.0
        self._tracking_angvel_reward_weight = 2.00
        self._feet_air_time_reward_weight   = 0.0
        self._foot_clearance_cost_weight    = -2.0
        self._foot_swing_height_cost_weight = -0.125

        self._angvel_xy_cost_weight         = -0.05
        self._angular_momentum_cost_weight = -0.02

        self._termination_cost_weight       = -000.0
        self._action_rate_cost_weight       = -0.10
        self._upright_reward_weight         = 1.0
        self._feet_slip_cost_weight         = -0.1
        self._pose_reward_weight            = 1.0
        self._soft_landing_cost_weight      = -1e-5
        ...
        self._feet_site_id = jp.array([
            self._mj_model.site("fl_foot").id,
            self._mj_model.site("fr_foot").id,
            self._mj_model.site("hl_foot").id,
            self._mj_model.site("hr_foot").id,
        ])

        self._target_foot_height = 0.10
    ...
    def reset(self, rng: jax.Array) -> mjx_env.State:
        ...
        info = {
            ...
            "contacts"           : jp.zeros(4, dtype=bool),
            "first_contact"      : jp.zeros(4, dtype=bool),
            "landing_peak"       : jp.zeros(4),
        ...
        metrics["cost/soft_landing"]      = jp.zeros(())
    ...
    def _calculate_done(self, data: mjx.Data) -> jax.Array:
        # Terminate episode based on orientation of robot relative to z axis and robot's height
        ...
        healthy_height      = torso_height > self._agent_healthy_height
        healthy             = jp.logical_and(healthy_orientation, healthy_height)
        return jp.where(healthy, 0.0, 1.0)
    ...
    def _get_reward(
        ...
        feet_air_time   = self._reward_feet_air_time(info)
        termination     = self._cost_termination(info)
        ...
        feet_slip       = self._cost_feet_slip(data, info)
        ...
        soft_landing      = self._cost_soft_landing(data, info)
        ...
        feet_air_time_reward   = feet_air_time * self._feet_air_time_reward_weight
        ...
        soft_landing_cost      = soft_landing * self._soft_landing_cost_weight
        ...
        metrics["reward/feet_air_time"]    = feet_air_time_reward
        ...
        metrics["cost/soft_landing"]       = soft_landing_cost
    ...
    def _reward_tracking_angvel(self, data, info) -> jax.Array:
        ...
        return jp.exp(-angvel_error / 0.5)
    ...
    def _cost_termination(self, info) -> jax.Array:
        return info["done"]

    def _reward_pose(self, data, info) -> jax.Array:
        """Variable posture reward matching mjlab's three-regime implementation.
        Joint order: fl_hx, fl_hy, fl_kn, fr_hx, fr_hy, fr_kn,
                     hl_hx, hl_hy, hl_kn, hr_hx, hr_hy, hr_kn
        """
        walking_threshold = 0.05
        running_threshold = 1.5

        # Per-joint stds: hx=0.3/0.5, hy=0.3/0.5, kn=0.15/0.2
        std_standing = jp.array([0.05] * 12)
        std_walking  = jp.array([0.3, 0.3, 0.15] * 4)
        std_running  = jp.array([0.5, 0.5, 0.2] * 4)
        ...
        std = jp.where(
            ...
            std_standing,
            jp.where(speed < running_threshold, std_walking, std_running),
        )
    ...
    def _cost_stand_still(self, data, info) -> jax.Array:
        command         = info["command"]
        cmd_norm        = jp.linalg.norm(command)
        joint_positions = data.qpos[self._agent_qpos_ids[7:]]
        cost = jp.sum(jp.abs(joint_positions - self._start_joints))
        cost *= cmd_norm < 0.01
        return cost
    ...
    def _reward_feet_air_time(
        ...
        in_range = jp.logical_and(
            feet_air_time > threshold_min,
            feet_air_time < threshold_max,
        )
        reward = jp.sum((in_range * first_contact).astype(jp.float32))
        total_command = jp.linalg.norm(command[:2]) + jp.abs(command[2])
        active = total_command > command_threshold
        return reward * active.astype(jp.float32)

    def _cost_foot_swing_height(
        self,
        info: dict[str, Any],
    ) -> jax.Array:
        command_threshold = 0.05
        landing_peak = info["landing_peak"]
        first_contact = info["first_contact"]
        command = info["command"]

        error = landing_peak / self._target_foot_height - 1.0
    ...
    def _cost_feet_slip(
        ...
        in_contact = contact.astype(jp.float32)
        total_command = jp.linalg.norm(command[:2]) + jp.abs(command[2])
        active = (total_command > command_threshold).astype(jp.float32)
        return jp.sum(vel_xy_norm_sq * in_contact) * active
    ...
    def _get_feet_contact_forces(self, data: mjx.Data) -> jax.Array:
        fl = mjx_env.get_sensor_data(self._mj_model, data, "fl_foot_floor_force")
        fr = mjx_env.get_sensor_data(self._mj_model, data, "fr_foot_floor_force")
        hl = mjx_env.get_sensor_data(self._mj_model, data, "hl_foot_floor_force")
        hr = mjx_env.get_sensor_data(self._mj_model, data, "hr_foot_floor_force")
        return jp.stack([fl, fr, hl, hr], axis=0)

    def _get_feet_vel(self, data: mjx.Data) -> jax.Array:
        front_left_foot_vel = mjx_env.get_sensor_data(self._mj_model, data, "fl_foot_global_linvel")
        front_right_foot_vel = mjx_env.get_sensor_data(self._mj_model, data, "fr_foot_global_linvel")
        back_left_foot_vel = mjx_env.get_sensor_data(self._mj_model, data, "hl_foot_global_linvel")
        back_right_foot_vel = mjx_env.get_sensor_data(self._mj_model, data, "hr_foot_global_linvel")
        return jp.stack([front_left_foot_vel, front_right_foot_vel, back_left_foot_vel, back_right_foot_vel], axis=0)
    ...
    def _cost_soft_landing(self, data, info) -> jax.Array:
        """Penalize high impact forces at landing to encourage soft footfalls."""
        forces = self._get_feet_contact_forces(data)
        force_magnitude = jp.linalg.norm(forces, axis=-1)  # [4]
        first_contact = info["first_contact"]
        landing_impact = force_magnitude * first_contact.astype(jp.float32)
        return jp.sum(landing_impact)
    ...
    def _get_feet_contacts(self, data: mjx.Data) -> jax.Array:
        return jp.array([
            mjx_env.get_sensor_data(self._mj_model, data, "fl_foot_floor_contact")[0] > 0,
            mjx_env.get_sensor_data(self._mj_model, data, "fr_foot_floor_contact")[0] > 0,
            mjx_env.get_sensor_data(self._mj_model, data, "hl_foot_floor_contact")[0] > 0,
            mjx_env.get_sensor_data(self._mj_model, data, "hr_foot_floor_contact")[0] > 0,
        ])
...
locomotion.register_environment('velocity_spot', VelocitySpot, default_config)
