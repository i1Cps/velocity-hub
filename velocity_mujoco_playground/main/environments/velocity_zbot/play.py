...
class VelocityZbot_Play:
    def __init__(self):
        options_path = "main/environments/velocity_zbot/options.json"
        xml_path     = "main/environments/velocity_zbot/model.xml"
        ...
        # Create agent brain
        brain_neurons = network_model.load_params("models/velocity_zbot/model")
        ...
        self._command_step_linvel_y    = 0.05
...
def _make_training_env(mjx_options: dict[str, Any]) -> VelocityZbot:
    env_cfg = mp_registry.get_default_config("velocity_zbot")
    env_cfg["impl"] = mjx_options["impl"]
    env_cfg["ctrl_dt"] = mjx_options["ctrl_dt"]
    env_cfg["sim_dt"] = mjx_options["sim_dt"]
    return VelocityZbot(config=env_cfg)
