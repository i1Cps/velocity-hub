...
class VelocityT1_Play:
    def __init__(self):
        options_path = "main/environments/velocity_t1/options.json"
        xml_path     = "main/environments/velocity_t1/model.xml"
        ...
        # Create agent brain
        brain_neurons = network_model.load_params("models/velocity_t1/model")
...
def _make_training_env(mjx_options: dict[str, Any]) -> VelocityT1:
    env_cfg = mp_registry.get_default_config("velocity_t1")
    env_cfg["impl"] = mjx_options["impl"]
    env_cfg["ctrl_dt"] = mjx_options["ctrl_dt"]
    env_cfg["sim_dt"] = mjx_options["sim_dt"]
    return VelocityT1(config=env_cfg)
